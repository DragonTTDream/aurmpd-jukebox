...
| 页面顶部的仓库署名抢眼、遮挡播放器、手机端占一整行 | 署名条在桌面端绝对定位在右上角（压在播放器上），窄屏无定位规则则掉入正常流占满一行 | 移入页脚并整体弱化（`.78em` + 55% 透明度）；桌面端贴底右对齐细条，窄屏正常流最末居中。滚动容器补 32px 让位避免遮住最后一行；右下角消息提示上移 36px 避免重叠 |
